...
void PyQt5QmlPlugin::getSipAPI()
{
    ...
    if (!sip) {
        ...
        if ((sip_sipmod = PyImport_ImportModule("PyQt5.sip")) == nullptr)
        {
            PyErr_Print();
        } else {
            ...
            if (sip_capiobj == nullptr || !PyCapsule_CheckExact(sip_capiobj))
            {
                PyErr_Print();
            } else {
                sip = reinterpret_cast<const sipAPIDef *>(PyCapsule_GetPointer(sip_capiobj, "PyQt5.sip._C_API"));
            }
        }
    }
